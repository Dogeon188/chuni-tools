...
export function DerivedStream<T extends Readable<unknown>[], R>(
	stores: T,
	callback: (values: {
		[K in keyof T]: T[K] extends Readable<infer U> ? U : never
	}) => Promise<R>,
	initial_value?: R | undefined
): Readable<R>
export function DerivedStream<T, R>(
	stores: Readable<T> | Readable<unknown>[],
	callback: (values: T | unknown[]) => Promise<R>,
	initial_value?: R | undefined
): Readable<R> {
