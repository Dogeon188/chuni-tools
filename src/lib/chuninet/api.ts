...
export async function fetchBestRecord(
	diff: Difficulty = Difficulty.master
): Promise<BestRecord[]> {
	...
	const recordList = Array.from(
		...
		.map((f) => {
			...
			return {
				...
				difficulty: diff,
				clear: icons?.querySelector(`img[src*="alljustice"]`)
					? 'AJ'
					: icons?.querySelector(`img[src*="fullcombo"]`)
						? 'FC'
						: '',
...
export async function fetchPlayHistory(): Promise<HistoryRecord[]> {
	...
	const historyRecord = Array.from(dom.querySelectorAll('.mt_10 .frame02.w400')).map(
		(d) => {
			const scoreStr = d.querySelector('.play_musicdata_score_text')?.innerHTML
			const diffSrc = (<HTMLImageElement>d.querySelector('.play_track_result img'))
				.src
			const diffStr = /musiclevel_.*(?=\.png)/.exec(diffSrc)![0].slice(11)
			const icons = <Array<HTMLDivElement>>(
				Array.from(d.querySelectorAll('.play_musicdata_icon'))
			)
			return {
				...
				difficulty:
					diffStr == 'ultimate'
						...
						: diffStr == 'worldsend'
							? 'WE'
							: Difficulty[<keyof typeof Difficulty>diffStr],
				clear: icons.some((di) => di.querySelector(`img[src*="alljustice"]`))
					? 'AJ'
					: icons.some((di) => di.querySelector(`img[src*="fullcombo"]`))
						? 'FC'
						: '',
...
export async function fetchRecentRecord(): Promise<PlayRecord[]> {
	...
	return Array.from(dom.querySelectorAll('form')).map((f) => {
		...
		return {
			title: f.querySelector('.music_title')!.innerHTML,
			score: parseNumber(f.querySelector('.text_b')?.innerHTML || '0'),
...
export async function fetchSongPlayCount(idx: string, diff: Difficulty) {
	...
	const pcStr = dom
		...
		.querySelector('.text_b')
		?.innerHTML.replace('times', '')
